grammar NET; // rename to distinguish from Expr.g4
...
classDefn returns [String str]
@init{StringBuilder bufferClassDefn = new StringBuilder();}
@after{$str = bufferClassDefn.toString();}
			: 'class' className '{' {bufferClassDefn.append("class " + $className.text + "\n{\n");} (classElement {bufferClassDefn.append($classElement.str + "\n");})* '}' {bufferClassDefn.append("}\n");}
			;
...
//instanceAttributes returns [String str]
//@init{StringBuilder bufferInstanceAttributes = new StringBuilder();}
//@after{$str = bufferInstanceAttributes.toString();}
// 	: (instanceAttribute {bufferInstanceAttributes.append($instanceAttribute.str + "\n");})*
// 	;
...
//				: inputBindings ',' {bufferInputBindings.append($inputBindings.str + ", ");}  inputBindings
//				| formalName '=' actualName {bufferInputBindings.append($formalName.text + " = " + $actualName.text);}
//				;
...
@after{$str = bufferInputBindings.toString();}
				: //inputBindings ',' {bufferInputBindings.append($inputBindings.str + ", ");}  inputBindings
				//formalName '=' actualName {bufferInputBindings.append($formalName.text + " = " + $actualName.text);} (',' inputBindings {bufferInputBindings.append(", " + $inputBindings.str);})*
...
//				:
//				outputBindings {bufferOutputBindings.append($outputBindings.str);} ',' outputBindings {bufferOutputBindings.append(", " + $outputBindings.str);}
//				| actualName '=' formalName {bufferOutputBindings.append($actualName.text + " = " + $formalName.text);}
//				;
...
@after{$str = bufferOutputBinding.toString();}
			:	formalName '=' actualName {bufferOutputBinding.append($actualName.text + " = " + $formalName.text);}
			;
...
//		: NUMBER {bufferTuple.append("Number " + $NUMBER.text + "\n");}
//		| '(' tuple ')' {bufferTuple.append("(tuple): ( "); if ($tuple.str != null) bufferTuple.append($tuple.str);  bufferTuple.append(" )");}
//		| tuple NUMBER {if ($tuple.str != null) bufferTuple.append("tuple Number " + $tuple.str); bufferTuple.append(" " + $NUMBER.text);}
//		| '(' tuple ')' {bufferTuple.append("(tuple) tuple  ( "); if ($tuple.str != null) bufferTuple.append($tuple.str); bufferTuple.append(" ) ");}
//		   tuple {if ($tuple.str != null) bufferTuple.append($tuple.str);}
//		;
...
/*
 * After left recursion is removed:
A: Aa | b;
 becomes
 A: bR;
R: (aA)?;
