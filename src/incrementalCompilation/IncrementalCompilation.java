package incrementalCompilation;
...
public class IncrementalCompilation {
	...
	public static void getDAG(Domain domain) throws ExceptionHugin{
		ArrayList<Node> nodeList= new ArrayList<Node>();
		nodeList = domain.getNodes();
		
		HashMap<String, ArrayList<Node>> dAG=new HashMap<String, ArrayList<Node>>();
		
		for(Node node: nodeList){
			System.out.println("Node name " + node.getName());
			System.out.println("Parents " + node.getParents());
			dAG.put(node.getName(), node.getParents());
		}
	}
	
	public static void displayDomainInfo(Domain domain) throws ExceptionHugin{
		ArrayList<Node> nodeList= new ArrayList<Node>();
		nodeList = domain.getNodes();
		
		// this for loop is for testing how to get internal graph structure
		// using node list and their parents you can get the whole graph.
		// You can add parents that mean adding edge is allowed
		
		for(Node node: nodeList){
			System.out.println("Node name " + node.getName());
			System.out.println("Ancestors " + node.getAncestors());
			System.out.println("Descendants " + node.getDescendants());
			System.out.println("Parents " + node.getParents());
			System.out.println("PDag Parents " + node.getPdagParents());
			System.out.println("PDag Neighbour " + node.getPdagNeighbors());
		}
